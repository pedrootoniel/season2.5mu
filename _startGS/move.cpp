...
void commandS::ChatProto(LPBYTE aRecv, int aIndex)
{
	...
	for(int i = 0; i <  this->moves.count; i++)
	{
		...
		{
			this->movestart(player,
			this->movesarray[i].vip,
			this->movesarray[i].pk,
			this->movesarray[i].level,
			this->movesarray[i].map,
			this->movesarray[i].cordx,
			this->movesarray[i].cordy);
		}
	}
