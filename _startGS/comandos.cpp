...
void _comandos::_comandos_config()
{
	  ...
	  GetPrivateProfileString("settings","iniciar_string[25]","vazio",_iniciar_comandos._struct->string[25],30,".\\configurações\\_administrador.settings\\_settings.comandos.ID");
	  GetPrivateProfileString("settings","iniciar_string[26]","vazio",_iniciar_comandos._struct->string[26],30,".\\configurações\\_administrador.settings\\_settings.comandos.ID");
	  ...
	  _iniciar_comandos._struct->money[24] = iniciar_dll.iniciar_getprivateint("settings","iniciar_money[24]", 0,".\\configurações\\_administrador.settings\\_settings.comandos.ID");
  	  ...
  	  _iniciar_comandos._struct->ativar[25]= iniciar_dll.iniciar_getprivateint("settings","iniciar_ativar[25]",0,".\\configurações\\_administrador.settings\\_settings.comandos.ID");
	  _iniciar_comandos._struct->level[25] = iniciar_dll.iniciar_getprivateint("settings","iniciar_level[25]", 0,".\\configurações\\_administrador.settings\\_settings.comandos.ID");
	  _iniciar_comandos._struct->vip[25]   = iniciar_dll.iniciar_getprivateint("settings","iniciar_vip[25]",   0,".\\configurações\\_administrador.settings\\_settings.comandos.ID");
	  _iniciar_comandos._struct->money[25] = iniciar_dll.iniciar_getprivateint("settings","iniciar_money[25]", 0,".\\configurações\\_administrador.settings\\_settings.comandos.ID");
	  	  
  	  _iniciar_comandos._struct->ativar[26]= iniciar_dll.iniciar_getprivateint("settings","iniciar_ativar[26]",0,".\\configurações\\_administrador.settings\\_settings.comandos.ID");
	  _iniciar_comandos._struct->level[26] = iniciar_dll.iniciar_getprivateint("settings","iniciar_level[26]", 0,".\\configurações\\_administrador.settings\\_settings.comandos.ID");
	  _iniciar_comandos._struct->vip[26]   = iniciar_dll.iniciar_getprivateint("settings","iniciar_vip[26]",   0,".\\configurações\\_administrador.settings\\_settings.comandos.ID");
	  _iniciar_comandos._struct->money[26] = iniciar_dll.iniciar_getprivateint("settings","iniciar_money[26]", 0,".\\configurações\\_administrador.settings\\_settings.comandos.ID");
...
void _comandos::_comando_money(LPOBJ lpObj)
{
	...
	MsgOutput(lpObj->m_Index,"%s Zen Adicionado com sucesso ",lpObj->Name);
}
...
//----------------------------------------------------------------------------------------//
//reload base
void _comandos::_comando_reload_(LPOBJ lpObj)
{
	//----------- inicia calcharacter
	...
	GCServerMsgStringSend("[Reload] Sistemas Recarregados",lpObj->m_Index,0);
...
//----------------------------------------------------------------------------------------//
// Offtrade
void _comandos::_comando_offtrade(int aIndex)
{
	OBJECTSTRUCT* lpObj = (OBJECTSTRUCT*)GS_GOBJPOINTER(aIndex);

	if(_iniciar_comandos._struct->ativar[25] == 0)
	{
		_mensagem(lpObj->m_Index,73,0,0,0);
		return;
	}
    if(lpObj->Level < _iniciar_comandos._struct->level[25])
	{
		_mensagem(lpObj->m_Index,74,_iniciar_comandos._struct->level[25],0,0);
		return;
	}
	if(lpObj->vip < _iniciar_comandos._struct->vip[25])
	{
		_mensagem(lpObj->m_Index,75,0,0,0);
	    return;
	}
    if(lpObj->Money < _iniciar_comandos._struct->money[25])
	{
		_mensagem(lpObj->m_Index,76,_iniciar_comandos._struct->money[25],0,0);
		return;
	}
	
	if(lpObj->MapNumber != 0)
	{
		MsgOutput(aIndex,"[OFFTrade][%s]Você precisa estar em Lorencia",lpObj->Name);
		return;
	}

	if(lpObj->m_bPShopOpen == false)
    {
        MsgOutput(aIndex,"[%s] Primeiro Abra a Loja Pessoal!",lpObj->Name);
        return;
    }
...
//----------------------------------------------------------------------------------------//
// Offline
void _comandos::_comando_offline(int aIndex)
{
	OBJECTSTRUCT* lpObj = (OBJECTSTRUCT*)GS_GOBJPOINTER(aIndex);

	if(_iniciar_comandos._struct->ativar[26] == 0)
	{
		_mensagem(lpObj->m_Index,73,0,0,0);
		return;
	}
    if(lpObj->Level < _iniciar_comandos._struct->level[26])
	{
		_mensagem(lpObj->m_Index,74,_iniciar_comandos._struct->level[26],0,0);
		return;
	}
	if(lpObj->vip < _iniciar_comandos._struct->vip[26])
	{
		_mensagem(lpObj->m_Index,75,0,0,0);
	    return;
	}
    if(lpObj->Money < _iniciar_comandos._struct->money[26])
	{
		_mensagem(lpObj->m_Index,76,_iniciar_comandos._struct->money[26],0,0);
		return;
	}
...
void _comandos::_comandos_protocolo(LPBYTE Protocolo, int aIndex)
{
	...
	if(!memicmp(&Protocolo[13],_iniciar_comandos._struct->string[25],strlen(_iniciar_comandos._struct->string[25])))
	{
	  this->_comando_offtrade(aIndex);
	}
	if(!memicmp(&Protocolo[13],_iniciar_comandos._struct->string[26],strlen(_iniciar_comandos._struct->string[26])))
	{
	  this->_comando_offline(aIndex);
	}
}
