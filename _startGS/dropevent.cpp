...
{
	Enable = iniciar_dll.iniciar_getprivateint("DropEvent","Ativar", 0, ".\\configurações\\_eventos.settings\\_settings.drop_event.ID");
...
void cDropEvent::LoadFile()
{
	...
	FILE *file = fopen(".\\configurações\\_eventos.settings\\_settings.drop_event.ID","r");

	if(file == NULL)
	{
		MessageBoxA(0,"_settings.drop_event.ID Load Error","CRITICAL ERROR",0);
		ExitProcess(1);
		return;
	}
...
{
	AllServerAnnounce("[Drop Event] Iniciado em Lorencia!");
